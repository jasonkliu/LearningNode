...
var stream = crypto.createDecipher(cipher, passphrase);
...
tar.on('entry', function (entry) {
  ...
  entry.pipe(hash).pipe(through(function write_line(hash_buffer) {
    var hasher = hash_buffer.toString('hex');
	process.stdout.write(hasher + ' ' + name + '\n')
  }));
});
